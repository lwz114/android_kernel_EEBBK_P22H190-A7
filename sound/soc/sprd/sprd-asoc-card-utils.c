			 ...
			 struct snd_soc_card **card)
{
	 ...
	 * management enables the amplifier on playback. */
	{
		...
		if (pa_num > 0) {
			struct snd_soc_aux_dev *pa_aux;
			struct snd_soc_codec_conf *pa_conf;

			pa_aux = devm_kzalloc(dev, sizeof(*pa_aux) * pa_num,
					      GFP_KERNEL);
			pa_conf = devm_kzalloc(dev, sizeof(*pa_conf) * pa_num,
					       GFP_KERNEL);
			if (pa_aux && pa_conf &&
			    soc_sia81xx_init(pdev, pa_aux, pa_num,
					     pa_conf, pa_num) == 0) {
				priv->snd_card.aux_dev = pa_aux;
