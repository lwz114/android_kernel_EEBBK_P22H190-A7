...
static int aw_dev_reg_update_mute(struct aw_device *aw_dev,
			struct aw_data_container *profile_data)
{
	int i = 0;
	int ret = -1;
	...
	for (i = 0; i < profile_data->len; i = i + 2) {
		...
		ret = aw_dev_i2c_write_byte(aw_dev, profile_data->data[i], reg_val);
		if (ret < 0)
			return ret;
	}
...
int aw_dev_pid_18_pwr_on(struct aw_device *aw_dev, struct aw_data_container *data)
{
	int ret = 0;
	...
	/* Update scene parameters in mute mode */
	ret = aw_dev_reg_update_mute(aw_dev, data);
	if (ret < 0)
		return ret;
