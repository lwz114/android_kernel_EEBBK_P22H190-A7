...
int mcdt_dac_dma_enable(unsigned int channel, unsigned int emptymark)
{
	...
	/* EEBBK A3: ASoC DPCM may call hw_params twice for one stream;
	 * if the channel is already enabled, reuse it instead of failing */
	if (g_dac_channel[channel].dma_enabled == 1) {
		dma_channel = g_dac_channel[channel].dma_channel;
		uid = mcdt_send_data_use_dma(channel, dma_channel);
		pr_info("%s %d channel %d already enabled, uid=%d\\n",
			__func__, __LINE__, channel, uid);
		return uid;
	}
...
int mcdt_adc_dma_enable(unsigned int channel, unsigned int fullmark)
{
	...
	/* EEBBK A3: idempotent for double hw_params calls */
	if (g_adc_channel[channel].dma_enabled == 1) {
		dma_channel = g_adc_channel[channel].dma_channel;
...
static int mcdt_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(agcp_ahb_gpr)) {
		pr_warn("ERR: [%s] Get the agcp ahb syscon failed! (%ld)\n",
