...
static int fts_check_bootid(void)
{
    ...
    if (((0x86 == id[0]) && (0x32 == id[1])) || ((0x86 == id[0]) && (0x42 == id[1]))||
...
int fts_enter_test_environment(bool test_state)
{
	...
	if (test_state) {
		...
		if(upg->ts_data->ic_type == IC_TYPE_FT8006S_AA)
        {
			...
			if(ft8006saa){
				printk("kaoshan ###%s %d FTS_SELF_TEST_BIN =%s \n",__func__,__LINE__,FTS8006SAA_SELF_TEST_BIN);
				ret = request_firmware(&fw, FTS8006SAA_SELF_TEST_BIN, upg->ts_data->dev);//upg->setting_nf->app2_offset;
			}
			else {
				printk("kaoshan ###%s %d FTS_SELF_TEST_BIN =%s \n",__func__,__LINE__,FTS_SELF_TEST_BIN);
				ret = request_firmware(&fw, FTS_SELF_TEST_BIN, upg->ts_data->dev);//upg->setting_nf->app2_offset;
			}
        }
        else
    	{
	    	 printk("kaoshan ###%s %d FTS8722_SELF_TEST_BIN =%s\n",__func__,__LINE__,FTS8722_SELF_TEST_BIN);
			 ret = request_firmware(&fw, FTS8722_SELF_TEST_BIN, upg->ts_data->dev);//upg->setting_nf->app2_offset;
    	}
...
static int fts_fwupg_get_module_info(struct fts_upgrade *upg)
{
	...
	printk("kaoshan info.file_name =%s \n ",info->vendor_name);
...
static int fts_get_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
    ...
    if(upg->ts_data->ic_type == IC_TYPE_FT8006S_AA)
    {
	     if(ft8006saa)
	     {
		     printk("kaoshan firmware(%s) request \n", FTS8006SAA_UPGRADE_FW_BIN);  
...
void  get_tp_fw_ver(void)
{
    ...
    if(fwver)
    {
      printk("kaoshan %s %d fwver = 0x%x\n",__func__,__LINE__,fwver);
      snprintf(buf, sizeof(buf), "%02x", fwver);
	sprintf(Ctp_name,"Holitech,FTS8006saa,FW:%02x", fwver);
	tp_register_devinfo(fwver);
    }
}
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
	...
	printk("kaoshan ts_data->ic_type = 0x%x\n",ts_data->ic_type);
