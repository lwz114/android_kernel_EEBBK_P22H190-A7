...
static inline void mali_freq_init(struct device *dev)
{
	...
	gpu_dvfs_ctx.aon_apb_reg.regmap_ptr = syscon_regmap_lookup_by_name(dev->of_node,"mem_repaired_flag");
	KBASE_DEBUG_ASSERT(gpu_dvfs_ctx.aon_apb_reg.regmap_ptr);
	syscon_get_args_by_name(dev->of_node,"mem_repaired_flag", 2, (uint32_t *)gpu_dvfs_ctx.aon_apb_reg.args);
