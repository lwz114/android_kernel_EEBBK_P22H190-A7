...
# Profiles are selected with P21H180_PROFILE and P21H180_DEFCONFIG. The
# default profile remains the original P21H180 build; Droidspaces uses:
#   P21H180_PROFILE=droidspaces
#   P21H180_DEFCONFIG=ums512-p21h180_droidspaces_defconfig
TARGET="${1:-all}"
...
case "$TARGET" in
    ...
    -h|--help|help)
        ...
        printf '    P21H180_PROFILE=droidspaces \\\n'
        printf '    P21H180_DEFCONFIG=ums512-p21h180_droidspaces_defconfig \\\n'
        printf '    %s all\n' "$0"
        exit 0
        ;;
    *)
...
SRC_DIR="${P21H180_SOURCE_DIR:-$(cd "$(dirname "$0")/.." && pwd -P)}"
PROFILE="${P21H180_PROFILE:-p21h180}"
DEFCONFIG="${P21H180_DEFCONFIG:-${DEFCONFIG:-ums512-p21h180_defconfig}}"
if [[ "$PROFILE" == "p21h180" ]]; then
    PROFILE_SUFFIX=""
else
    PROFILE_SUFFIX="-$PROFILE"
fi
OUT_DIR="${P21H180_BUILD_DIR:-$SRC_DIR/out/ums512-p21h180${PROFILE_SUFFIX}}"
DEST_DIR="${P21H180_OUTPUT_DIR:-$SRC_DIR/out/release${PROFILE_SUFFIX}}"
...
SOCKO_TEMPLATE="${SOCKO_TEMPLATE:-$SRC_DIR/prebuilts/p21h180/socko.factory.img}"
ANYKERNEL_DIR="${ANYKERNEL_DIR:-$SRC_DIR/out/AnyKernel3}"
RECOVERY_SCRIPT_TEMPLATE="${RECOVERY_SCRIPT_TEMPLATE:-$SRC_DIR/scripts/p21h180-recovery-anykernel.sh.in}"
...
KERNEL_LOCALVERSION="-lwztime-build-g$GIT_HASH"
if [[ "$PROFILE" != "p21h180" ]]; then
    KERNEL_LOCALVERSION="-lwztime-$PROFILE-build-g$GIT_HASH"
fi
KBUILD_BUILD_USER="${KBUILD_BUILD_USER:-lwztime}"
KBUILD_BUILD_HOST="${KBUILD_BUILD_HOST:-P21H180-build}"
...
STAGE="$(mktemp -d /tmp/p21h180-output.XXXXXX)"
...
RECOVERY_OUTPUT="$DEST_DIR/P21H180-recovery${PROFILE_SUFFIX}-$KERNEL_RELEASE.zip"
